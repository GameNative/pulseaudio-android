...
static int pa_create_aaudio_stream(struct userdata *u) {
    ...
    if (res != AAUDIO_OK) {
        LOGW("AAudioStreamBuilder_openStream() failed.");
        return -1;
    }
    
    res = AAudioStreamBuilder_delete(u->builder);
    if (res != AAUDIO_OK) {
        LOGW("AAudioStreamBuilder_delete() failed.");
        return -1;
    }
    ...
    LOGW("AAudio stream opened: %d Hz, %d channels, format %d, sharing mode %s",
    ...
    if (u->low_latency) {
        ...
        if (bufferSize > bufferCapacity) {
            ...
            LOGW("AAudio: Requested buffer size exceeds capacity, clamped to %d frames", bufferSize);
        }
        
        res = AAudioStream_setBufferSizeInFrames(u->stream, bufferSize);
        if (res < 0) {
            LOGW("AAudioStream_setBufferSizeInFrames() failed: %d", res);
        } else {
            LOGW("AAudio buffer size set to %d frames (burst: %d, capacity: %d)", 
                   bufferSize, u->frames_per_burst, bufferCapacity);
        }
        
        u->previous_underrun_count = 0;
    } else {
        LOGW("AAudio low latency mode disabled, using default buffer size (burst: %d, capacity: %d)", 
...
static void try_start_stream(struct userdata *u) {
    const aaudio_result_t res = AAudioStream_requestStart(u->stream);
    if (res == AAUDIO_OK) {
...
static void recreate_aaudio_stream(struct userdata *u) {
    ...
    if (pa_create_aaudio_stream(u) < 0) {
        LOGW("Failed to create AAudio stream, scheduling recreate");
        schedule_recreate(u);
    } else {
        LOGW("AAudio stream created, attempting to start");
        schedule_start(u);
    }
}
...
static int sink_process_render(struct userdata *u, void *audioData, int64_t numFrames) {
    ...
    if (stream_state == AAUDIO_STREAM_STATE_PAUSED || 
        stream_state == AAUDIO_STREAM_STATE_PAUSING ||
        stream_state == AAUDIO_STREAM_STATE_STOPPED ||
        stream_state == AAUDIO_STREAM_STATE_STOPPING) {
        pa_silence_memory(audioData, u->frame_size * numFrames, &u->ss);
        return AAUDIO_CALLBACK_RESULT_CONTINUE;
    }
...
static int sink_process_msg(pa_msgobject *o, int code, void *data, int64_t offset, pa_memchunk *memchunk) {
    ...
    if (code == SINK_MESSAGE_TRY_START) {
        LOGW("AAudio try start requested");
        if (PA_SINK_IS_OPENED(u->sink->thread_info.state)) {
            try_start_stream(u);
        } else {
            LOGW("Sink no longer in opened state, canceling try");
        }
        return 0;
    }
    
    if (code == SINK_MESSAGE_RECREATE) {
        LOGW("AAudio recreate requested");
        if (PA_SINK_IS_OPENED(u->sink->thread_info.state)) {
            recreate_aaudio_stream(u);
        } else {
            LOGW("Sink no longer in opened state, canceling recreate");
        }
        return 0;
    }
...
static int sink_set_state_io_thread(pa_sink *s, pa_sink_state_t state, pa_suspend_cause_t suspend_cause) {
    ...
    LOGW("AAudio sink state transition: current=%d, target=%d, suspend_cause=%d", 
           s->thread_info.state, state, suspend_cause);

    if (PA_SINK_IS_OPENED(s->thread_info.state) && state == PA_SINK_UNLINKED) {
        LOGW("AAudio stream stopping for unlink");
        schedule_stop(u);
    } else if (PA_SINK_IS_OPENED(s->thread_info.state) && state == PA_SINK_SUSPENDED) {
        LOGW("AAudio stream stopping for suspend");
        schedule_stop(u);
    } else if (s->thread_info.state == PA_SINK_SUSPENDED && PA_SINK_IS_OPENED(state)) {
        aaudio_stream_state_t stream_state = AAudioStream_getState(u->stream);
        LOGW("AAudio stream resuming from suspended state, stream state: %d", stream_state);

        if (stream_state == AAUDIO_STREAM_STATE_PAUSED ||
            stream_state == AAUDIO_STREAM_STATE_PAUSING) {
            LOGW("AAudio stream paused / pausing, trying to start");
            schedule_start(u);
        } else if (stream_state == AAUDIO_STREAM_STATE_STOPPED ||
            stream_state == AAUDIO_STREAM_STATE_STOPPING) {
            LOGW("AAudio stream stopped / stopping, trying to start");
            schedule_start(u);
        } else if (stream_state != AAUDIO_STREAM_STATE_STARTED && stream_state != AAUDIO_STREAM_STATE_STARTING) {
            LOGW("AAudio stream not running (state: %d), recreating and starting", stream_state);
            schedule_recreate(u);
        }
    } else if (s->thread_info.state == PA_SINK_INIT && PA_SINK_IS_OPENED(state)) {
        aaudio_stream_state_t stream_state = AAudioStream_getState(u->stream);
        LOGW("AAudio stream initial start, state: %d", stream_state);
...
int pa__init(pa_module* m) {
    ...
    if (pa_modargs_get_value_boolean(ma, "low_latency", &u->low_latency) < 0) {
        LOGW("Failed to parse low_latency argument.");
        goto error;
    }
    ...
    if (!u->sink) {
        LOGW("Failed to create sink object.");
        goto error;
    }
